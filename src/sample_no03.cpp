...
{
    public:
        BehaviorTreeNode () : Node ( "sample_no03" )
        {
            ...
            m_Factory.registerNodeType<FullWordCountJudge> ( "FullWordCountJudge" );
            m_Factory.registerNodeType<HalfWordCountJudg> ( "HalfWordCountJudg" );
            m_Factory.registerNodeType<Display> ( "Display" );
            m_Factory.registerNodeType<SendText> ( "SendText" );

            string package_path = ament_index_cpp::get_package_share_directory ( "bt_sample" );            
            m_Factory.registerBehaviorTreeFromFile( package_path + "/config/sample_no03.xml" );
            m_tree = m_Factory.createTree ( "SampleNo03" );
